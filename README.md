...
├── api_client.py        # Класс с методами API-вызовов
...
```bash
pytest                        # все тесты
pytest test_authentication.py           # только тесты аутентификации
pytest test_profiles.py       # только тесты профилей
pytest -v                     # с подробным выводом
```

## Описание тестов

### `test_authentication.py` — Аутентификация
...
| `test_verify_fake_token` | Запрос с неправильным токеном |
| `test_verify_blank` | Запрос без токена |
| `test_replacement_password` | Смена пароля и вход с новым паролем |
| `test_login_different_roles` | Тест с параметрами: вход для user, admin, moderator |

### `test_profiles.py` — Профили и роли

| Тест | Описание |
|------|----------|
| `test_user_get_his_profile` | Пользователь получает свой профиль |
| `test_user_validation_profile_via_pydantic` | Структура профиля правильная |
